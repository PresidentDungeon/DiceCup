...
class MainActivity : AppCompatActivity() {
    ...
    fun onClickImage(view: View){
        ...
        val historyLast: Int = if(history.size > 0) history[history.size-1][placement] - 1 else 0
    ...
    fun onClickRoll(view: View) {
        ...
        if(!shouldRoll.contains(false)){
            ...
            for(i in allButtons.indices) {
                ...
                if(number == -1 && history.size > 0){currentHistory[i] = history[history.size-1][i]}
    ...
    private fun updateHistory() {
        var historyString = ""
        history.forEach{ array -> array.forEach { value -> historyString += "$value," }; historyString = historyString.removeRange(historyString.lastIndex,historyString.lastIndex+1); historyString += "\n" }
